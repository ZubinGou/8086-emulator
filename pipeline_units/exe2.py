...
class execution_unit(object):
    def __init__(self, BIU):
        ...
        self.eu_regs = list(self.reg.keys()) + [
    ...
    def stack_related_ins(self):
        if self.opcode == 'PUSH':
            self.reg['SP'] -= 2
            self.write_mem(self.bus.reg['SS'] * 16 + self.reg['SP'], \
                self.get_int(self.opd[0]))
        elif self.opcode == 'POP':
            res_list = self.bus.read_word(self.bus.reg['SS'] * 16 + self.reg['SP'])
            res = 0
            for num in res_list:
                res = (res << 8) + int(num, 16)
            ...
            self.reg['SP'] -= 2
            self.write_mem(self.bus.reg['SS'] * 16 + self.reg['SP'], self.FR.get_int())
